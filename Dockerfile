...
ENV FLINK_TGZ_URL=https://www.apache.org/dyn/closer.cgi?action=download&filename=${FLINK_URL_FILE_PATH} \
    ...
    GCS_CONNECTOR_URL=https://storage.googleapis.com/hadoop-lib/gcs/gcs-connector-1.6.2-hadoop2.jar
...
RUN set -ex; \
  ...
  rm flink.tgz; \
  \
  wget -nv -O /opt/flink/lib/gcs-connector-1.6.2-hadoop2.jar "$GCS_CONNECTOR_URL"; \
  \
  chown -R flink:flink .;
